...
import { ChevronLeft, Save, Plus, Trash2, Edit2 } from 'lucide-vue-next';
import { ref } from 'vue';
...
const form = useForm({
    ...
    estimated_duration_minutes: props.activity.estimated_duration_minutes,
    difficulty_level: props.activity.difficulty_level,
    min_age: props.activity.min_age,
    max_age: props.activity.max_age,
    is_active: props.activity.is_active,
    order: props.activity.order,
});
...
const addItem = () => {
    useForm({
        ...
        prompt_text: newItem.value.prompt_text,
        max_points: newItem.value.max_points,
        time_limit_seconds: newItem.value.time_limit_seconds,
        order: newItem.value.order,
    }).post(route('admin.activities.items.store', props.activity.id), {
        onFinish: () => {
            newItem.value = {
                item_type: 'emoji_choice',
                prompt_text: '',
...
const deleteItem = (itemId) => {
    if (confirm('Are you sure you want to delete this item?')) {
        useForm({}).delete(route('admin.activities.items.destroy', itemId), {
            onFinish: () => {
                window.location.reload();
            },
        });
...
const deleteActivity = () => {
    if (confirm('Are you sure you want to delete this activity?')) {
        useForm({}).delete(route('admin.activities.destroy', props.activity.id), {
            onFinish: () => {
            ...
            <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
                    ...
                    <div class="p-6 border-b border-gray-200 dark:border-gray-700">
                        <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">Edit Activity</h2>
                    ...
                    <div class="p-6">
                        <h3 class="text-xl font-bold text-gray-900 dark:text-white mb-6">Activity Items</h3>
                        ...
                        <!-- Items List -->
                        <div v-if="activity.activity_items && activity.activity_items.length > 0" class="space-y-3">
                            <h4 class="font-medium text-gray-900 dark:text-white">Current Items:</h4>
                            <div v-for="item in activity.activity_items" :key="item.id" class="bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg flex justify-between items-start">
                                <div class="flex-1">
                                    ...
                                    <p class="text-gray-900 dark:text-white font-medium">{{ item.prompt_text }}</p>
                        ...
                        <div v-else class="text-center py-8 text-gray-500 dark:text-gray-400">
                            <p>No items yet. Add items using the form above.</p>
                        </div>
                    </div>
                </div>
            </div>
